    ...
    <div ref={containerRef} className="absolute top-4 left-4 z-10 w-72">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-zinc-700 bg-zinc-900/90 backdrop-blur-sm pl-10 pr-4 py-2.5 text-sm text-zinc-100 placeholder-zinc-500 focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/30"
        />
        {query && (
          <button
      ...
      {/* Results dropdown */}
      {showResults && (
        <div className="mt-1 max-h-80 overflow-y-auto rounded-lg border border-zinc-700 bg-zinc-900/95 backdrop-blur-sm shadow-xl">
          {/* Filter tabs */}
          <div className="flex border-b border-zinc-700 px-1 py-1 gap-1">
            {([
              ['all', 'All'],
              ['photos', 'With Photos'],
              ['no-photos', 'No Photos'],
          ...
          <div className="py-1">
            {filtered.length === 0 ? (
              <div className="px-4 py-3 text-sm text-zinc-500">No towns found</div>
            ) : (
              filtered.map(town => (
                <button
                  key={town.name}
                  onClick={() => handleSelect(town)}
                  className="flex w-full items-center gap-3 px-4 py-2 text-left text-sm hover:bg-zinc-800 transition-colors"
                >
                  <span className={`h-2 w-2 rounded-full flex-shrink-0 ${
                    town.hasPhotos ? 'bg-amber-400' : 'bg-zinc-600'
                  }`} />
                  <span className="text-zinc-200">{town.name}</span>
          ...
          {/* Result count */}
          <div className="border-t border-zinc-700 px-4 py-1.5 text-xs text-zinc-500">
